...
struct Vector {
  ...
  constexpr size_t set_size(size_t size) {
    return stbds_arrsetlen(_data, size);
  }
...
struct Hashmap {
  ...
  constexpr bool has(KEY key) {
    return stbds_hmgetp_null(_data, key) != nullptr;
  }

  constexpr const VALUE &get_or_default(KEY key) {
    return stbds_hmget(_data, key);
  }
  ...
  constexpr VALUE put(KEY key, VALUE value) {
    return stbds_hmput(_data, key, value);
  }
